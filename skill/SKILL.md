...
用户可直接说：**“用 loop 优化当前项目：<目标>，测试通过后让网页版复审，直到验收。”**
默认沿用当前项目、原执行 owner 和已授权范围。主持任务负责请求封装、网页收发、
测试和回执，不能把 JSON、CLI 参数或日志搬运变成用户的必填步骤。
用户只说“继续”时恢复当前目标；用户给出新目标时新建 taskId，不能继承历史 DONE
或 EXECUTED 作为本轮结果。范围不明确先做有界只读规划，不擅自修改别人的工作树。

优先读取已有 `c2c session` 地址并恢复那个对话。项目已绑定且带有 Sources 的
Work 对话可以用于只读规划审查；不要因为另一个普通 Chat 不支持 MCP 就放弃它。
先在已绑定对话实际调用 workspace_info 并与本地匹配，再进入源码规划。Work 的
标签本身不构成成功证据，也不授权网页执行命令、改文件或另起 agent。
本地执行回执写入后，请同一对话实际读取对应 taskId/iteration 的 execution_summary；
核对实际结果后继续审查，不能只相信网页复述了主持者发去的摘要。
连接器若指向主工作树而执行在隔离树，必须核验工具实际读到的版本；不能以主树
的 diff 代替隔离树审查。沿用既有受支持的只读产物入口，无法匹配则明确保留缺口。
...
- 命令返回 session_id/process handle 且仍 running 时，把原句柄、cwd、任务与迭代
  写入既有 checkpoint.next；使用原执行工具的 wait/write_stdin 获取后续结果。
  超时不是失败，不重新启动测试或另建执行 session。句柄不可用时先核对进程和产物，
  结果未知保持未完成；不得猜测 exit 0。每次只读新增输出或有界尾部。
- 一轮修改完成后集中获取一次 diff 与测试摘要，再发 REVIEW。优先使用当前工具
  已返回的 reviewRef/提交版本/差异入口；不为了审查再创建工作树或第二套账本。
  HEAD 相同不代表未提交内容相同：若审查后文件又变更，旧审查不能覆盖新改动。
  工具返回截断时明确列缺失文件，定向补读，不把局部片段当作完整审查。
- 发给网页的反馈只包含目标、采纳项、本轮变更摘要、真实测试结果、剩余问题和
  已核验的证据入口。长日志留本地，通过已授权只读工具按需读取。同一缺陷没有
  新证据时先定位原因，再决定补测、研究或修改；不要机械重复同一计划。
- 用户询问状态时给出“阶段、最近完成动作、待处理项、下一步”四项简短信息。
  连接正常、通知已出现、Agent 已接受任务均不能替代执行终态或网页审查结果。

### 其他 MCP 后端

Agent Helm / DevSpace 仅作为可选接入路径。相关调研与使用映射见
`__C2C_CHECKOUT__/docs/loop-backends.md`，只有用户要求这些后端或当前通道确实
存在缺口时才读取。先发现实际暴露的工具与版本，不凭 README 猜测已安装。
保留现有 C2C 通道；新后端安装、权限或执行 owner 的变化不能由网页提案自行授权。
一个目标只有一个执行者。插件支持多 Agent 不等于当前任务授权启动多 Agent。
